...
//cerr << "checkConsistencyOfTiledImageGeometry(): is WSI so perform check" << endl;
		...
		Uint16 vInConcatenationTotalNumber = 0;	// optional, and absence means we don't know how many there are
		{
			Attribute *aInConcatenationTotalNumber=list[TagFromName(InConcatenationTotalNumber)];
			if (aInConcatenationTotalNumber) {
				(void)aInConcatenationTotalNumber->getValue(0,vInConcatenationTotalNumber);
			}
		}
